...
class TRADE(nn.Module):
    def __init__(self, mode, hidden_size, lang, path, task, lr, dropout, slots, gating_dict, nb_train_vocab=0):
        ...
        self.decoder = Generator(self.mode, self.lang, self.encoder.embedding, self.lang.n_words, hidden_size, self.dropout, self.slots, self.nb_gate)
    ...
    def save_model(self, dec_type):
        ...
        directory = 'save/TRADE-'+args["addName"]+args['dataset']+str(self.task)+'-' + str(args["value_oov_rate"]) + '-' + args['mode'] +'/'+'HDD'+str(self.hidden_size)+'BSZ'+str(args['batch'])+'DR'+str(self.dropout)+str(dec_type)
    ...
    def train_batch(self, data, clip, slot_temp, reset=0):
        ...
        loss_ptr = masked_cross_entropy_for_value(
            ...
            data["y_out_lengths"]) #[:,:len(self.point_slots)])
        '''
        #print('all_point_outputs.size()',all_point_outputs.size())
        #print('all_point_outputs',all_point_outputs)
        ...
        for j, data_dev in pbar:
            ...
            for bi in range(batch_size):
                ...
                if set(data_dev["turn_belief"][bi]) == set(predict_belief_bsz_ptr) and args["genSample"]:
                    utterance=data_dev["context_plain"][bi]
                    us=utterance.split(';')
                    for ui in range(len(us)):
                        ...
                        f.write(u)
                        f.write('\n')
                    True_state='True_state: '+str(set(data_dev["turn_belief"][bi]))
                    f.write(True_state)
                    f.write('\n')
                    pred_state='pred_state: '+str(set(predict_belief_bsz_ptr))
                    f.write(pred_state)
                    f.write('\n')
                    f.write('\n')
                    #print('utterance',data_dev["context_plain"][bi])
                    #print("True", set(data_dev["turn_belief"][bi]) )
                    #print("Pred", set(predict_belief_bsz_ptr), "\n")
                '''
                '''
                utterance = data_dev["context_plain"][bi]
                pred_state=';'.join(predict_belief_bsz_ptr)
                s1='\t<=>\t'.join([utterance,pred_state])
                f.write(s1)
                f.write('\n')

                gold_state = ';'.join(data_dev["turn_belief"][bi])
                s1 = '\t<=>\t'.join([utterance, gold_state])
                f1.write(s1)
                f1.write('\n')
                '''
        #f.close()
...
class Generator(nn.Module):
    def __init__(self, mode, lang, shared_emb, vocab_size, hidden_size, dropout, slots, nb_gate):
    ...
    def forward(self, batch_size, encoded_hidden, encoded_outputs, encoded_lens, story, max_res_len, target_batches,\
        ...
        for slot in slot_temp:
            ...
            if self.mode == 'cat':
                decoder_input = self.dropout_layer(slot_emb).expand(batch_size, self.hidden_size * 2)
            else:
            ...
            for wi in range(max_res_len):
                ...
                if extra_zeros is not None:
                    p_vocab = torch.cat([p_vocab, extra_zeros], 1)
                ...
                final_p_vocab = final_p_vocab + 1e-12
                ...
                if self.mode=='pws':
                    decoder_input = (1 - vocab_pointer_switches) * decoder_input1 + \
                    vocab_pointer_switches * decoder_input2
